...
# Exploration des représentations par packages
...
Peut-être homogéneiser car dorling avec minuscule et Olsaon avec majuscule
...
## Hybride Voronoi et Dougenick

A vous de jouer !
Quel package pour créer une tesselation de la zone d'étude
Fichier entrée : les centroïdes des entités et le contour de la zone pour la découpe. 
Le fond Voronoi peut ensuite être transformé par l'un des alorithme qui créer le cartogramme continu
...
extraction des centroïdes sous la forme d'une table XY
```{r, eval = TRUE, message = FALSE, warning = FALSE, results = FALSE}
...
Création d'un fichier pour qu'il soit pile poil conforme à ce que veut Recmap
```{r, eval = TRUE, message = FALSE, warning = FALSE, results = FALSE}
...
Un truc je sais pas à quoi ça sert ...
```{r, eval = TRUE, message = FALSE, warning = FALSE, results = FALSE}
map <- checkerboard
```
...
Pas sûre qu'il faille le mettre parce que si on veut s'en tenir au traitement carto de la donnée, il ne sert à rien dans le cadre de la production de connaissances scientifiques.

Eduardo Dutenkefer a exploré l'esthétique des cartogrammes et notamment imaginé l'anti-cartogramme, qui inverse le poids des entités : la surface varie en proportion inverse à la valeur cartographiée. XXXXX comment on dit ça ???? Tu as pigé ?". Sur le plan scientifique, cette représentation n'a pas de sens car ne peut être interprétée. En effet, le cartogramme révèle les poids des lieux et la variation de taille a un sens bien défini en cartographie, basé sur la perception visuelle (la surface permet d'estimer la part de chaque ou d'un groupe d'entités sur le total). Visualiser le "non poids" des lieux a vocation à soutenir un discours artistique, métaphorique sur un espace et non pas à cartographier une variable quantitative. Pour s'intéresser aux zones à faible effectif, le traitement de la donnée s'effectue par le filtrage. Cf. en carto des flux, vous ne représentez pas les anti flux ou le flux le plus insignifiant devient le plus massif ... Vous filtrez

Quel serait l'objectif d'un anti cartogram, qu'est-ce que c'est censé montrer ?
...
Calcul de la valeur du "non péhnomène" cartographié (1/)
```{r AntiCartogram, eval = TRUE, message = FALSE, warning = FALSE}
communes$inverse = 1/communes$pop2018
```

```{r AntiCartogram2, eval = TRUE, message = FALSE, warning = FALSE}
...
Nico, est-ce que il ne faudrait pas être plutôt sur la population 18-65 pour calculer un taux à partir de csp ? Bref, base communes à affiner pour que l'on soit sur d'être "carré" sur les variables derivée (cf. pour le chomage, il faut la population active).
```{r, eval = TRUE, message = FALSE, warning = FALSE, results = FALSE}
...
Dorling = cartogram_dorling(communes, "pop2018", k = 1.8)
```
Comparaison entre différents fonds de carte pour le taux de cadres : géographique, cartogramme continu, discontinu ou de Dorling
...
>>>>>>> f49a13d560d6d9de8bed525d40b1bf01e42e2143
