...
## Rappel en carto "classique" : mapsf
...
##  Déplacer des disques : vers le cartogramme de Dorling avec packcircles
...
Judith Olson, en réaction aux premiers cartogrammes continus de Tobler et notamment les probèmes de lisibilité, propose une variation proportionnelle de la surface tout en gardant sa forme "géographique" [@olson1976noncontiguous]
...
```{r Olsen2, eval=TRUE, message=FALSE, warning=FALSE, results=}
...
# Les equal density cartogram de Gastner et al. avec cartogramR

```{r, eval = TRUE, message = FALSE, warning = FALSE}
library(cartogramR)
```

Le package cartogramR est déveoppé par Pierre-Andre Cornillon et Florent Demoraes de l'université de Rennes. Le package propose les méthodes *flow based cartogram* [@gastner2004diffusion] implanté dans Scapetoad, *fast flow based cartogram* [@gastner2018fast] et *rubber band based cartogram* [@dougenik1985algorithm]. La méthode flow based cartogram est basée sur [go_cart](https://github.com/Flow-Based-Cartograms/go_cart).
...
# Bonus : de multiples formes pour augmenter les points de vues
...
## Anti Cartogramme
Eduardo Dutenkefer a exploré l'esthétique des cartogrammes et notamment imaginé l'anti-cartogram, qui inverse les poids des entités : la surface varie inversement proprtionnellement à la valeur cartographiée. XXXXX comment on dit ça ????"
