...
Versus comparaison par la forme qui reproduit en général la répartition de la population lorsque l'on est sur des données démographiques

```{r, eval = TRUE, message = FALSE, warning = FALSE, results = FALSE}
communes$cadr <- round(communes$cadr)
communes$ouvr <- round(communes$ouvr)
doug_cadr <- cartogram_cont(communes, "cadr")
doug_ouvr <- cartogram_cont(communes, "ouvr")
par(mfrow = c(1, 2), mar = c(0,0,0,0))
mf_map(doug_cadr)
mf_map(doug_ouvr)
```
