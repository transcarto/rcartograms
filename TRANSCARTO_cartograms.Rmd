...
NB : Pour installer le package *cartogramR*, vous aurez besoin d'installer au préalable [FFTW](http://www.fftw.org/). Sous Windows, cette opération est loin d'être simple...
...
**Le package [mapsf](https://riatelab.github.io/mapsf/)** permet de faire des cartes thématiques dans R. C'est le package qui succède au package *cartography* (qui est en mode maintenance).
...
grid = st_sf(idgrid = c(1:length(grid)), geometry = grid)
ctr = st_centroid(grid)
# On effectue une jointure spatiale (point within polygon)
point_in_poly <- st_intersection(ctr, communes[,c("id", "pop2018")])
# On compte le nombre de points par commune
n_pts_com <- aggregate(point_in_poly$id, by= list(id=point_in_poly$id), length)
# On récupère le nombre de point par commune
ctr <- merge(point_in_poly, n_pts_com, by = "id", all.x = TRUE)
# On affecte les valeurs aux points
ctr$pop <- ctr$pop2018 / ctr$x
# On récupère les données dans la grille
grid <- merge(grid, st_drop_geometry(ctr[,c("idgrid","pop")]), 
              by = "idgrid", all.y = TRUE)
# Export des données
...
**Le package [packcirles](https://github.com/mbedward/packcircles) propose 3 algorithmes simples pour déplacer des disques sur un plan 2D de telle sorte qu'ils ne se supperposent pas.** Il reprend le principe du cartogramme de Dorling et permet de gérer la superposition des cercles, améliorant la lisibilité générale de la carte par symboles proportionnels [@dorling1996area].
...
**le package [cartogram](https://github.com/sjewo/cartogram)** est développé par Sebastian Jeworutzki. Il propose trois méthodes : le cartogramme **circulaire**  de Dorling, **discontinu** d'Olson et **continu** de Dougenik, Chrisman et Niemeyer).
...
<img src ="img/Dorling_exemple.png"></img>
...
Judith Olson, en réaction aux premiers cartogrammes continus de Tobler et notamment les problèmes de lisibilité, propose une variation proportionnelle de la surface des entités tout en gardant la forme "géographique" [@olson1976noncontiguous]
...
James A. Dougenik, Nicholas R. Chrisman et Duane R.Niemeyer, sur la base des premiers travaux de W. Tobler, développent en 1985 un des deux algorithmes les plus implanté dans les outils (avec celui de Gastner et Newman développé en 2004). [@dougenik1985algorithm][@dorling_new_1995]

x	: un objet sf.

weight : om de la variable.

itermax	: nombre d'itérations maximum tant que maxSizeError n'est pas atteint.

maxSizeError : la déformation s'arrête si l'erreur moyenne est inférieure à cette valeur.

prepare	: mettre "adjust" permert d'accélérer le temps de calcul.
...
## Les algorithmes pour les cartogrammes continus avec le package cartogramR
...
```{r gridcartogram2, eval = TRUE, message = FALSE, warning = FALSE, results = FALSE}
...
Il s'agit d'articuler une simplification des tracés des unités administratives en articulant une partition de Voronoï (de préférence calculé sur les centre des communes si l'on travaille sur les population) et le cartogramme continu. 

NB : calculer les polygones de Voronoï en R est un peu *tricky*. [Voir](https://github.com/r-spatial/sf/issues/474) et [voir](https://stackoverflow.com/questions/45719790/create-voronoi-polygon-with-simple-feature-in-r).
...
Comme nous ne disposons pas de la population active, on se basera sur la population générale.
```{r, eval = TRUE, message = FALSE, warning = FALSE, results = FALSE}
