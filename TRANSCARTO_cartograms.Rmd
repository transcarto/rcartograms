...
Le package **mapsf** permet de faire des carte sthématiques dans R.
...
coords <- data.frame(st_coordinates(st_centroid(st_geometry((x)))))
bb <- lapply(st_geometry(x), function(x){st_bbox(x)})
dx <- unlist(lapply(bb, function(x){x[3]-x[1]})) * 0.5
dy <- unlist(lapply(bb, function(x){x[4]-x[2]})) * 0.5
...
df <- data.frame(x = coords$X, 
                 ...
                 z = x$pop2008,
                 name = x$id)
par(mar = c(0,0,0,0), bg = "lemonchiffon", mfrow = c(1,3))
plot(st_geometry(x), col = "ivory2")

plot.recmap(df, col = 'ivory2', border = "black")

cartog <- recmap(df)
plot(cartog,  col = 'ivory2', border = "black")
