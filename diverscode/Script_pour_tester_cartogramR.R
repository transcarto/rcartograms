...
View(france_dept)
# pour choisir la taille de la grille de d?formation (calcul un peu long)
...
france_cartogram_pop <- cartogramR(france_dept, count="pop2018", method="GastnerSeguyMore", options=list(L=256, grid=TRUE))
...
plot(final_graticule, border = "grey90", lwd = 1, lty =3) # affichage un peu long

# pour afficher le cartogramme
plot(france_cartogram_pop, border = "red", add = TRUE)
plot(final_graticule, border = "grey90", lwd = 1, lty =3, add= TRUE) # affichage un peu long
...
# pour exporter le cartogramme au format geopackage (attention deux champs ne peuvent avoir le m?me nom)
